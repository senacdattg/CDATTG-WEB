...
import type { PersonalAdministrativoImportLogItem, PersonalAdministrativoImportResult } from '../types';

const ACCEPTED_FORMATS = '.xlsx,.xls';
const EXTENSIONES_PERMITIDAS = new Set(['xlsx', 'xls']);

export const ImportarPersonalAdministrativo = () => {
  const [file, setFile] = useState<File | null>(null);
  const [importing, setImporting] = useState(false);
  const [importError, setImportError] = useState('');
  const [lastResult, setLastResult] = useState<PersonalAdministrativoImportResult | null>(null);
  const [imports, setImports] = useState<PersonalAdministrativoImportLogItem[]>([]);
  ...
  const fetchImports = useCallback(async () => {
    try {
      ...
      const data = await apiService.getPersonalAdministrativoImports(50);
      setImports(data);
    } catch {
      setImports([]);
    } finally {
  ...
  const handleStartImport = async () => {
    ...
    setLastResult(null);
    try {
      const result = await apiService.uploadPersonalAdministrativoImport(file);
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const blob = await apiService.downloadPersonalAdministrativoImportTemplate();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'plantilla_importar_personal_administrativo.xlsx';
      a.click();
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-gray-600 dark:text-gray-400">
        Carga masiva de personal administrativo desde Excel. Se crean personas si no existen y se vinculan como personal administrativo.
      </p>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card">
            ...
            <div className="mt-4 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <label htmlFor="imp-pa-archivo" className="sr-only">
          ...
          <div className="card">
            ...
            <ul className="mt-3 space-y-2 text-gray-600 dark:text-gray-400 text-sm list-disc list-inside">
              ...
              <li>Si la persona no existe se crea; si ya es personal administrativo se cuenta como duplicado y no se genera error.</li>
            </ul>
          </div>
        </div>
          ...
          <div className="mt-3 overflow-x-auto">
            ...
            {!loadingImports && imports.length > 0 && (
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-600 text-sm">
                <caption className="sr-only">Historial de importaciones de personal administrativo</caption>
