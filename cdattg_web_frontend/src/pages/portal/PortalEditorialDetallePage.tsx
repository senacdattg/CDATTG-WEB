...
/**
 * Cuerpo, adjuntos y datos de convocatoria o actividad.
 */
    ...
    <main className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      ...
      {item ? (
        <article className="card space-y-3">
          ...
          {item.audio_url ? (
            <a className="text-primary-700 hover:underline" href={portalMediaUrl(item.audio_url)}>
          ...
          {item.enlace_externo ? <a className="text-primary-700 hover:underline" href={item.enlace_externo} rel="noreferrer" target="_blank">Enlace externo</a> : null}
        </article>
      ) : null}
    </main>
  );
